...
class DynamicSky extends Mesh {

	constructor() {

		const shader = DynamicSky.SkyShader;

		const material = new ShaderMaterial( {
			name: 'DynamicSkyShader',
		...
		super( new PlaneGeometry( 200, 100 ), material );

		this.isDynamicSky = true;
...
const texture = textureLoader.load(noise, (tex) => {
    tex.wrapS = tex.wrapT = RepeatWrapping;
});

DynamicSky.SkyShader = {

	uniforms: {
        iTime: {value:0},
        iChannel0:{value: texture},
        iResolution: { type: 'v2', value: new Vector2(window.innerWidth, window.innerHeight) },
        iMouse:{type: "vec2", value:new Vector2(0,0)}
      },
